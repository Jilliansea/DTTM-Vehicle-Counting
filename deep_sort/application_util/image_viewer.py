...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def __init__(self, output_file, seq, update_ms=5, window_shape=(640, 480), caption="Figure 1"):
        self._window_shape = window_shape
        self._caption = caption
        self._update_ms = update_ms
        # self._video_writer = None
        self.video_writer = cv2.VideoWriter(
            os.path.join(output_file+'/avi', seq+'.avi'), cv2.VideoWriter_fourcc(*"XVID"), 10, self._window_shape)
    ...
    def polygen(self, pts):
        cv2.polylines(self.image, [pts], True, (0, 255, 0), self.thickness)

    def trajectory(self, pts):
        for i in range(1, len(pts)):
            if pts[i - 1] is None or pts[i] is None:
                continue
            # cv2.line(self.image, pts[i - 1], pts[i], self._color, self.thickness)
            cv2.line(self.image, tuple([int(i) for i in list(pts[i - 1])]), tuple([int(i) for i in list(pts[i])]),
    ...
    def enable_videowriter(self):
        ...
        self._video_writer.write(self.image)
    ...
    def run(self, update_fun=None):
        ...
        while not self._terminate:
            ...
            # cv2.imwrite('/Users/didi/Desktop/ReID/deep_sort/save_file/'+str(t1)+'.jpg',self.image)
